"""
Create Cohort 3 tables in the PUBLIC PostgreSQL schema using a cohort_3_ prefix.

All table names:
    cohort_3_user_pii
    cohort_3_user_pii_injected
    cohort_3_bob_companies
    cohort_3_credit_links
    cohort_3_skillboost_profile
    cohort_3_skilllab_submission
    cohort_3_codelab_submission
    cohort_3_project_submission
    cohort_3_optional_mcq_verification
    cohort_3_optional_mcq_response
    cohort_3_main_mcq_response
    cohort_3_sync_state

View:
    cohort_3_user_pii_combined  (UNION of cohort_3_user_pii + cohort_3_user_pii_injected)

Run from the project root:
    python server/migrations/create_cohort3_tables.py
...
CREATE TABLE IF NOT EXISTS {PREFIX}user_pii (
    ...
    display_registered_at TIMESTAMP,
...
_ALL_DDL = [
    ...
    (_DDL_SYNC_STATE,                f"{PREFIX}sync_state"),
]
...
def run_migration(database_url: str) -> None:
    ...
    print(f"\n[DONE] All Cohort 3 tables created with prefix '{PREFIX}' in public schema.")
...
def main() -> None:
    print(f"=== Creating Cohort 3 tables (prefix: '{PREFIX}') in public schema ===\n")
