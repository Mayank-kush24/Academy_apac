"""
Seed cohort_2_credit_links with the five Skill Lab catalog URLs (2000 max allocations each).

Idempotent: inserts only when cohort_2_credit_links has no rows.

Run from project root:
    python server/migrations/seed_cohort2_credit_links.py
...
def main():
    ...
    with engine.begin() as conn:
        n = conn.execute(text("SELECT COUNT(*) FROM cohort_2_credit_links")).scalar()
        if n and int(n) > 0:
            print(f"cohort_2_credit_links already has {n} row(s); skipping seed.")
            return
        for link_url, display_order, max_allocations in COHORT2_CREDIT_LINKS:
            conn.execute(
                text(
                    """
                    INSERT INTO cohort_2_credit_links (link_url, display_order, max_allocations)
    ...
    print("Seeded cohort_2_credit_links: 5 links, 2000 max allocations each.")
