...
def get_portal_dashboard_url() -> str:
    """CDI portal dashboard URL (same target as post-logout redirect)."""
    return f"{get_portal_url().rstrip('/')}/dashboard"
...
def _first_allowed_path(pages: list[str] | None) -> str:
    """Redirect target for L2 allow-list (prefer home, then dashboard, then others)."""
    from server.cdi_integration import parse_cohort_portal_page_id, portal_page_id_to_app_path
    ...
    def _abs(rel: str) -> str:
        ...
        if rel == "/":
            return f"{_root}/" if _root else "/"
        return (_root + rel) if _root else rel
    ...
    for logical in priority:
        for p in pages:
            if p == logical:
                return _abs(portal_page_id_to_app_path(p))
        for p in pages:
            _, lg = parse_cohort_portal_page_id(p)
            if lg == logical:
                return _abs(portal_page_id_to_app_path(p))
    return _abs(portal_page_id_to_app_path(pages[0]))
...
def _enforce_request_auth(page: str | None):
    ...
    if page and not payload.get("isAdmin"):
        ...
        if pages is not None:
            from server.cdi_integration import portal_allowlist_allows_resolved_page

            if not pages:
                _root = (request.environ.get("SCRIPT_NAME") or "").rstrip("/")
                return redirect(f"{_root}/" if _root else "/")
            if not portal_allowlist_allows_resolved_page(pages, page):
...
def register_with_portal(
    ...
    pages: list of dicts with at least {"pageId", "label", "path"}; optional keys (e.g. "group")
    are forwarded for portal UI (cohort-wise grouping).
    """
